...
(function () {
  ...
  function createMin(parent) {
    min = Utils("div", {
      ...
      backgroundImage: "url(".concat(frist, ")"),
      backgroundSize: "100% 100%" // border: "1px solid #CCCCCC",
  ...
  function createImageCon(parent) {
    ...
    for (var i = 0; i < iconList.length; i++) {
      var img = Utils("img", {
        ...
        border: "2px solid rgba(255,0,0,".concat(i == 0 ? 1 : 0, ")"),
        marginLeft: "".concat(i === 0 ? '0px' : IMAGE_MARGIN + "px"),
  ...
  function iconMouseHandler(e) {
    ...
    preImg = e.target;
    preImg.style.border = "2px solid rgba(255,0,0,1)"; //    console.log( e.target.src.replace("_icon",""));

    min.style.backgroundImage = max.style.backgroundImage = "url(".concat(e.target.src.replace(/_icon/, ""), ")");
  } // 移动 / 事件
  ...
  function clickHandler(e) {
    ...
    if (index === 0) {
      pos--;
      if (pos < 0) pos = 0; //    imgCon.style.left="0px";
    } else {
